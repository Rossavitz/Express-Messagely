...
class User {
  ...
  static async authenticate(username, password) {
    ...
    if (bcrypt.compare(password, user.password) === true) {
      return user;
    } else {
  ...
  static async messagesFrom(username) {
    const results = await db.query(
      `
      SELECT m.id, m.to_user, m.body, m.sent_at, m.read_at, u.first_name, u.last_name, u.phone
      FROM messages AS m
      JOIN users AS u
      ON m.to_username = u.username
      WHERE from _username = $1`,
      [username]
    );
  }
